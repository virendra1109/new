"""FAISS Indexers with hybrid keyword + semantic search."""
import faiss
import numpy as np
import pickle
...
class BaseFAISSIndexer:
    """Base class for FAISS indexing with persistent caching."""
    ...
    def _save(self, index_path: Path, metadata_path: Path, data: Any) -> None:
...
class FAISSToolIndexer(BaseFAISSIndexer):
    ...
    def __init__(self, server_name: str, tools: List[dict]) -> None:
        super().__init__()
    ...
    def search(self, query: str, top_k: int = 3) -> List[dict]:
        """Search tools using hybrid keyword + semantic matching."""
        if not self.tools:
            return []
        ...
        # Return keyword matches if sufficient
        ...
        # Combine keyword matches + semantic search
